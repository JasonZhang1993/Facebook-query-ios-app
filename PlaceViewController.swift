...
class PlaceViewController: UIViewController, UITableViewDataSource, UITableViewDelegate {
    ...
    override func viewDidLoad() {
        ...
        loc = SearchKeyword.center
        savedID = UserDefaults.standard.array(forKey: "placeid") as? [String] // only read userDefault
        search(for: "q")
    }
    
    override func viewWillAppear(_ animated: Bool) {
        savedID = UserDefaults.standard.array(forKey: "placeid") as? [String]
        self.placeTable.reloadData()
        self.placeTable.tableFooterView = UIView()
    }
    
    func search(for keyword: String) {
//        print(SearchKeyword.keyword) // testing
        if keyword == "q" {
//            searchURL = "http://cs-server.usc.edu:11549/hw9/hw8.php?q=\(q)&type=place&center=\(loc)"
            searchURL = "http://myapp2017-env.us-west-2.elasticbeanstalk.com/hw9/hw9.php?q=\(q)&type=place&center=\(loc)"
        }
    ...
    func tableView(_ tableView: UITableView, cellForRowAt indexPath: IndexPath) -> UITableViewCell {
        let cell = tableView.dequeueReusableCell(withIdentifier: "PlaceTableViewCell") as! SearchTableViewCell
